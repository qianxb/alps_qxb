...
int icn85xx_prog_i2c_txdata(unsigned int addr, char *txdata, int length)
{
  ...
  if (!i2c_client||!icn85xx_i2c_dma_va)
	...
	//tmp_buf[0] = (u8) ((addr >> 16) & 0xFF);//(unsigned char)(addr>>16);
	//tmp_buf[1] = (u8) ((addr >> 8) & 0xFF);//(unsigned char)(addr>>8);
	//tmp_buf[2] = (u8) (addr & 0xFF);//(unsigned char)(addr);
...
int icn85xx_i2c_rxdata(unsigned short addr, char *rxdata, int length)
{
	...
	u8 *r_buf = NULL;
      ...
      mutex_lock(&i2c_data_mutex);
      return -1;
      r_buf = kzalloc(length, GFP_KERNEL); 
      if (r_buf == NULL) 
      return -1; 
	//	{
...
//	node = of_find_matching_node(node, touch_of_match);
	...
	icn85xx_trace("[%s]irq:%d, debounce:%d-%d: ret = %d", __func__, touch_irq, ints[0], ints[1],ret);
	return ret;
}
...
static void tpd_resume(struct device *h)
{
    ...
    {
          icn85xx_trace("icn85xx_ts_resume zeni 666666\n");
    	icn85xx_write_reg(ICN85xx_REG_PMODE, 0xff);
	    //icn85xx_ts_reset();
    }
              icn85xx_trace("icn85xx_ts_resume zeni 77777\n");
