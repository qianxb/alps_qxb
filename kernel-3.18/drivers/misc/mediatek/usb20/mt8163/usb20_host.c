...
void mt_usb_set_vbus(struct musb *musb, int is_on)
{
	...
	if (is_on) {
		...
		gpio_direction_output(drvvbus_pin, 1);
		printk("****%d Drive VBUS ON!!!!!\n", __LINE__);
#endif
#else
...
#endif 
		ret = gpio_request_one(drvvbus_pin, GPIOF_OUT_INIT_LOW,
				 "drvbus");
	if (ret < 0) {
		dev_err(mtk_musb->controller,"kite Unable to request gpio drvvbus_pin\n");
	}
...
#endif	
       ret = gpio_request_one(iddig_pin, GPIOF_IN,
				 "id_int");
        if (ret < 0) {
		dev_err(mtk_musb->controller,"kite Unable to request gpio iddig_pin \n");
	}
