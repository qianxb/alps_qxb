...
static void android_view_RenderNode_requestPositionUpdates(JNIEnv* env, jobject,
    ...
    public:
        ...
        virtual ~SurfaceViewPositionUpdater() {
            /// M:Wait until all tasks finish. @{
            { // scope for the mutex
                AutoMutex _lock(mSyncMutex);
                mWaitToDie = true;
                while (mFunctorCount > 0) {
                    ALOGD("~SurfaceViewPositionUpdater dtor, remaining mFunctorCount(%d) <%p>",
                            mFunctorCount, this);
                    mSyncCondition.wait(mSyncMutex);
                }
            }
            ALOGD("~SurfaceViewPositionUpdater unlock <%p>", this);
            /// @}
    ...
    private:
        ...
        void doUpdatePosition(jlong frameNumber, jint left, jint top,
            ...
            if (CC_UNLIKELY(!localref)) {
                jnienv()->DeleteWeakGlobalRef(mWeakRef);
                mWeakRef = nullptr;
                ALOGW("SurfaceViewPositionUpdater doUpdatePosition return <%p>", this);
                signal();
                return;
            }
        ...
        void doNotifyPositionLost(jlong frameNumber) {
            ...
            if (CC_UNLIKELY(!localref)) {
                jnienv()->DeleteWeakGlobalRef(mWeakRef);
                mWeakRef = nullptr;
                ALOGW("SurfaceViewPositionUpdater doNotifyPositionLost return <%p>", this);
                signal();
                return;
            }
        ...
        /// M: Release lock and decrease counter
        void signal() {
            Mutex::Autolock l(mSyncMutex);
            mFunctorCount--;
            mSyncCondition.signal();
        }

        /// M: Check is in wait state or not
        bool isWaitToDie(const char* doWhat) {
            Mutex::Autolock l(mSyncMutex);
            if (mWaitToDie) {
                ALOGW("SurfaceViewPositionUpdater wait to die - %s, return directly. "
                        "mFunctorCount(%d) <%p>", doWhat, mFunctorCount, this);
                mFunctorCount--;
                mSyncCondition.signal();
                return true;
            }
        ...
        /// M: Fix race condition between gc-thread and worker-thread. @{
        int mFunctorCount;
        bool mWaitToDie;
        Mutex mSyncMutex;
