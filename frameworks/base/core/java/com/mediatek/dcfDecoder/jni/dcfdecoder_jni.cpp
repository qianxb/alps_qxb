...
static jobject decodeDcfSource(JNIEnv* env, jobject options,
                               ...
                               sp<DecryptHandle> decryptHandle ) {
    if (NULL == env) {
        ALOGE("decodeDcfSource: invalid JNIEnv.");
        return NULL;
    }
    if (NULL == drmManagerClient || NULL == decryptHandle.get()) {
        ALOGE("decodeDcfSource: drmManagerClient or decryptHandle is NULL");
        return NULL;
    }
    ...
    if (NULL == decryptedBuf) {
        ALOGE("decodeDcfSource: failed to decrypt DCF.");
        return NULL;
    }
    ...
    if (NULL == decoder) {
        ALOGE("decodeDcfSource: SkImageDecoder-Factory() failed, return NULL");
        free(decryptedBuf);
        return NULL;
    }
    ...
    if (NULL != options && env->GetBooleanField(options, options_mCancelID)) {
        ALOGE("decodeDcfSource: Decoding is cancelled, return NULL");
        free(decryptedBuf);
        return NULL;
    }
    ...
    // set allocator to decoder for M change
    JavaPixelAllocator javaAllocator(env);
    decoder->setAllocator((SkBitmap::Allocator*)&javaAllocator);
    ...
    if (!decoder->decode(&memStream, bitmap, prefConfig, decodeMode)) {
        ALOGE("decodeDcfSource: SkImageDecoder-decode() failed, return NULL");
        free(decryptedBuf);
        return NULL;
    }
    ...
    if (SkImageDecoder::kDecodeBounds_Mode == mode) {
        ALOGD("decodeDcfSource: justBounds mode, return NULL");
        return NULL;
    }

    if (bitmap != NULL) {
        ALOGD("decodeDcfSource: bitmap valid, wrap it into a Java Bitmap");
        // get bitmap from allocator for M change, and release this skbitmap after
        // return java bitmap
        jobject jbitmap = GraphicsJNI::createBitmap(env, javaAllocator.getStorageObjAndReset(),
                GraphicsJNI::kBitmapCreateFlag_Premultiplied, NULL);
        delete bitmap;
        return jbitmap;
    } else {
        ALOGD("decodeDcfSource: bitmap invalid");
        return NULL;
    }
}
...
static jobject nativeDecodeFile(JNIEnv* env, jobject clazz,
                                ...
                                jobject options,
                                jboolean consume) {
    ALOGD("nativeDecodeFile ---->");
    if (NULL == env || NULL == filePath) {
        ALOGE("nativeDecodeFile: JNIEnv or filePath is NULL");
        return NULL;
    }

    const char* pathname = env->GetStringUTFChars(filePath, NULL);
    ALOGD("nativeDecodeFile: decode file %s", pathname);
    if (NULL == pathname) {
        ALOGE("nativeDecodeFile: failed to get file name");
        return NULL;
    }
    ...
    if (0 > fd) {
        ALOGE("nativeDecodeFile: failed to open file");
        return NULL;
    }

    DrmManagerClient *drmManagerClient = new DrmManagerClient();
    if (NULL == drmManagerClient) {
        ALOGE("nativeDecodeFile: failed to create DrmManagerClient");
        close(fd);
        return NULL;
    }
    ...
    if (NULL == decryptHandle.get()) {
        ALOGE("nativeDecodeFile: failed to open decryptSession");
    ...
    if (bmp != NULL && consume == JNI_TRUE) {
        ALOGD("nativeDecodeFile: consume rights");
...
static jobject nativeForceDecodeFile(JNIEnv* env, jobject clazz,
                                     ...
                                     jobject options,
                                     jboolean consume) {
    ALOGD("nativeForceDecodeFile ---->");
    if (NULL == env || NULL == filePath) {
        ALOGE("nativeForceDecodeFile: JNIEnv or filePath is NULL");
        return NULL;
    }

    const char* pathname = env->GetStringUTFChars(filePath, NULL);
    ALOGD("nativeForceDecodeFile: decode file %s", pathname);
    if (NULL == pathname) {
        ALOGE("nativeForceDecodeFile: failed to get file name");
        return NULL;
    }
    //!!!!!! whether pathname needs to be freed when returned ?!!!!!!!!
    ...
    int fd = open(pathname, O_RDONLY);
    if (0 > fd) {
        ALOGE("nativeForceDecodeFile: failed to open file");
        return NULL;
    }

    DrmManagerClient *drmManagerClient = new DrmManagerClient();
    if (NULL == drmManagerClient) {
        ALOGE("nativeForceDecodeFile: failed to create DrmManagerClient");
        close(fd);
        return NULL;
    }
    ...
    if (NULL == decryptHandle.get()) {
        ALOGE("nativeForceDecodeFile: failed to open decryptSession");
        delete drmManagerClient;
        close(fd);
        return NULL;
    }
    ...
    jobject bmp = decodeDcfSource(env, options, drmManagerClient, decryptHandle);
    if (bmp != NULL && consume == JNI_TRUE) {
        ALOGD("nativeForceDecodeFile: consume rights");
        drmManagerClient->consumeRights(decryptHandle, Action::DISPLAY, false);
    }
...
static jbyteArray nativeDecryptFile(JNIEnv* env, jobject clazz,
                                    jstring filePath,
                                    jboolean consume) {
    ALOGD("nativeDecryptFile ---->");
    if (NULL == env) {
        ALOGE("nativeDecryptFile: invalid JNI env.");
        return NULL;
    }
    if (NULL == filePath) {
        ALOGE("nativeDecryptFile: invalid filePath.");
        return NULL;
    }
    ...
    const char* pathname = env->GetStringUTFChars(filePath, NULL);
    if (NULL == pathname) {
        ALOGE("nativeDecryptFile: failed to get file path name.");
        return NULL;
    }
    ALOGD("nativeDecryptFile: attempt to decrypt file [%s]", pathname);

    int fd = open(pathname, O_RDONLY);
    if (fd < 0) {
        ALOGE("nativeDecryptFile: failed to open file to read.");
        return NULL;
    }
    ...
    DrmManagerClient *drmManagerClient = new DrmManagerClient();
    if (NULL == drmManagerClient) {
        ALOGE("nativeDecryptFile: failed to create DrmManagerClient.");
        close(fd);
        return NULL;
    }
    sp<DecryptHandle> decryptHandle =
    ...
    if (decryptHandle.get() == NULL) {
        ALOGE("nativeDecryptFile: failed to open decrypt session.");
        delete drmManagerClient;
        close(fd);
        return NULL;
    }
    ...
    unsigned char* decryptedBuf = decryptDcfSource(env, drmManagerClient, decryptHandle, &length);
    if (NULL == decryptedBuf) {
        ALOGE("nativeDecryptFile: failed to decrypt DCF.");
        drmManagerClient->closeDecryptSession(decryptHandle);
        delete drmManagerClient;
        close(fd);
        return NULL;
    }
    ALOGD("nativeDecryptFile: result length [%d]", length);

    if (consume == JNI_TRUE) {
        ALOGD("nativeDecryptFile: consume rights.");
        drmManagerClient->consumeRights(decryptHandle, Action::DISPLAY, false);
    }
...
static jbyteArray nativeForceDecryptFile(JNIEnv* env, jobject clazz,
    ...
    if (NULL == env) {
        ALOGE("nativeForceDecryptFile: invalid JNI env.");
        return NULL;
    }
    if (NULL == filePath) {
...
static jbyteArray nativeDecryptDcfFile(JNIEnv* env, jobject clazz, jobject fileDescriptor,
    ...
    JNINativeMethod* gMethods, int numMethods)
{
    ...
    if (clazz == NULL) {
        ALOGE("registerNativeMethods: Native registration unable to find class '%s'",
                className);
        return JNI_FALSE;
    }
    if (env->RegisterNatives(clazz, gMethods, numMethods) < 0) {
        ALOGE("registerNativeMethods: RegisterNatives failed for '%s'", className);
        return JNI_FALSE;
    }
    return JNI_TRUE;
}
...
jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
    ...
    if (register_mediatek_DcfDecoder(env) < 0) {
        ALOGE("JNI_OnLoad: ERROR: register_mediatek_DcfDecoder failed\n");
        return result;
    }
