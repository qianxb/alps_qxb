...
public abstract class ActivityManagerNative extends Binder implements IActivityManager
{
    ...
    public boolean onTransact(int code, Parcel data, Parcel reply, int flags)
        ...
        case SUPPRESS_PACKAGES_TRANSACTION: {
            data.enforceInterface(IActivityManager.descriptor);
            List<String> packageList = null;
            data.readStringList(packageList);
            int suppressPolicy = data.readInt();
            String tag = data.readString();
            suppressPackages(packageList, suppressPolicy, tag);
            reply.writeNoException();
            return true;
        }

        case UNSUPPRESS_PACKAGES_TRANSACTION: {
            data.enforceInterface(IActivityManager.descriptor);
            String tag = data.readString();
            unsuppressPackages(tag);
            reply.writeNoException();
            return true;
        }
...
class ActivityManagerProxy implements IActivityManager
{
    ...
    public void suppressPackages(List<String> packageList, int suppressPolicy, String tag)
        ...
        data.writeInt(suppressPolicy);
        data.writeString(tag);
        mRemote.transact(SUPPRESS_PACKAGES_TRANSACTION, data, reply, 0);
        reply.readException();
        data.recycle();
        reply.recycle();
    }

    public void unsuppressPackages(String tag) throws RemoteException {
        Parcel data = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        data.writeInterfaceToken(IActivityManager.descriptor);
        data.writeString(tag);
        mRemote.transact(UNSUPPRESS_PACKAGES_TRANSACTION, data, reply, 0);
        reply.readException();
        data.recycle();
        reply.recycle();
    }
