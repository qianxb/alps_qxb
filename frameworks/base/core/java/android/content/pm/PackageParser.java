...
public class PackageParser {
            ...
            if (name != null) {
                ...
                if (index == -1) {
                    ...
                    if (CtaUtils.isCtaSupported()) {
                        addCtaOnlyPermissions(pkg, name.intern());
                    }
                    ///@}
                } else {
    ...
    /// M: CTA requirement - permission control  @{
    private void addCtaOnlyPermissions(Package pkg, String perm) {
        if (!CtaPermissions.MAP.containsKey(perm)) {
            return;
        }
        for (String ctaPerm : CtaPermissions.MAP.get(perm)) {
            int index = pkg.requestedPermissions.indexOf(ctaPerm);
            if (index == -1) {
                pkg.requestedPermissions.add(ctaPerm);
            }
        }
    }
    ///@}
