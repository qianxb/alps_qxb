    ...
    private class MiniThumbDataFile {
        ...
        public MiniThumbDataFile(RandomAccessFile miniThumbFile, String path) throws IOException {
            ...
            try {
                ...
                Log.w(TAG, "map MiniThumbFile(READ_WRITE) with NonWritableChannelException, try READ_ONLY mode", ex1);
                try {
            ...
            Log.v(TAG, "Create MiniThumbDataFile with size " + mRandomAccessFile.length() / 1024 + "KB");
        }
        ...
        public synchronized int getActiveCount() throws IOException {
            ...
            if (mIndexMappedBuffer.get(mVersionHeader) != null
                ...
                Log.v(TAG, "getActiveCount is " + mActiveCount);
                return mActiveCount;
            }
            Log.v(TAG, "invalid version header, reset MiniThumbDataFile");
            reset();
            return mActiveCount;
        }
        ...
        public synchronized long getMagic(long id) throws IOException {
            ...
            if (storedId == id) {
                Log.v(TAG, "getMagic succuss with: id = " + id + ", magic = " + magic);
                return magic;
            } else {
                Log.v(TAG, "getMagic fail for id " + id + " with store id is " + storedId);
                return 0;
            }
        }
        ...
        public synchronized void updateDataToThumbFile(byte[] data, long id, long magic) throws IOException {
            if (data == null || data.length > BYTES_PER_MINTHUMB) {
                Log.v(TAG, "updateDataToThumbFile with invalid data");
                return;
            }
            ...
            try {
                ...
                try {
                    ...
                    Log.e(TAG, "updateDataToThumbFile: can not release lock!", ex);
                }
            }
            Log.v(TAG, "updateDataToThumbFile succuss with " + bufferToString(mIndexHeader));
        }
        ...
        public synchronized byte[] getDataFromThumbFile(byte[] data, long id, ThumbResult result) throws IOException {
            if (getIndexHeader(mIndexHeader, id) == null) {
                Log.w(TAG, "can not get index header for id " + id);
                return null;
            }
            ...
            if (oldId != id) {
                Log.w(TAG, "invalid store original id : store id = " + oldId + ", given id = " + id);
                return null;
            }
            if (data.length < length) {
                Log.w(TAG, "invalid store data length: store length = " + length + ", given length = " + data.length);
                return null;
            }
            ...
            try {
                ...
                if (mChannel.read(mBuffer, position) >= length) {
                    ...
                    if (dataCheckSum == checkSum(data, 0 , length)) {
                        ...
                        Log.v(TAG, "getDataFromThumbFile success with " + bufferToString(mIndexHeader));
                        return data;
                    } else {
                        if (result != null) {
                ...
                try {
                    ...
                    Log.e(TAG, "getDataFromThumbFile: can not release lock!", ex);
                }
            }
            Log.v(TAG, "getDataFromThumbFile fail with " + bufferToString(mIndexHeader));
            return null;
        }

        public synchronized void close() {
            Log.v(TAG, "close MiniThumbDataFile " + mPath);
            syncAll();
        ...
        public void syncIndex() {
            try {
                ...
                Log.w(TAG, "sync MiniThumbDataFile index failed", t);
            }
        }
        ...
        public void syncAll() {
            ...
            try {
                ...
                Log.w(TAG, "sync MiniThumbDataFile failed", t);
            }
        }
