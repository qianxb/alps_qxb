...
public class DimLayer {
    private static String TAG = TAG_WITH_CLASS_NAME ? "DimLayer" : TAG_WM;
    ...
    private void constructSurface(WindowManagerService service) {
        ...
        /// M: Using surface name as log tag. {@
        TAG = getSurfaceName();
        /// @}
        try {
    ...
    /// M: Refine surface name.
    static boolean mIsAnimBg = false;
    private String getSurfaceName() {
        String surfaceName;
        if (!mIsAnimBg) {
            surfaceName = "DimLayer-" + mName;
            mIsAnimBg = true;
        } else {
            surfaceName = "DimLayer-" + mName + "-AnimBg";
            mIsAnimBg = false;
        }
        return surfaceName;
    }
