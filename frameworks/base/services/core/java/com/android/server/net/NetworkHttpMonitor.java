...
public class NetworkHttpMonitor {
    ...
    private static final int EXPIRE_TIME = 60 * 1000 * 20; //30; // 30 minutes
    ...
    private void enableFirewallWithUid(int appUid, boolean isEnabled) {
        try {
            if (isEnabled) {
                mNetd.setFirewallUidRule(FIREWALL_CHAIN_NONE, appUid, FIREWALL_RULE_ALLOW);
            }
            else {
                mNetd.setFirewallUidRule(FIREWALL_CHAIN_NONE, appUid, FIREWALL_RULE_DENY);
            }
    ...
    private void enableFirewall() {
        try {
            ...
            mNetd.setFirewallUidRule(FIREWALL_CHAIN_NONE, 0, FIREWALL_RULE_ALLOW);  //Root
            mNetd.setFirewallUidRule(FIREWALL_CHAIN_NONE, 1000, FIREWALL_RULE_ALLOW);  //System
...
/*
        if ((cm.getActiveLinkProperties().getInterfaceName()) != null) {
            NetworkUtils.resetConnections(cm.getActiveLinkProperties().getInterfaceName()
