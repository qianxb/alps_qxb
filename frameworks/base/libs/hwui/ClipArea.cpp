    ...
    if (!mLastResolutionResult
        ...
        if (CC_LIKELY(mMode == ClipMode::Rectangle
            ...
            case ClipMode::Region:
                ...
                /// M: Transform region clip area properly. @{
                SkPath pathFromRegion;
                bool ret = other.getBoundaryPath(&pathFromRegion);
                if (ret) {
                    SkPath pathTransformed;
                    SkMatrix skRecordedClipTransform;
                    recordedClipTransform.copyTo(skRecordedClipTransform);
                    // Transform path with parent transform matrix.
                    pathFromRegion.transform(skRecordedClipTransform, &pathTransformed);
                    // Set path back to region.
                    regionFromPath(pathTransformed, other);
                } else {
                /// @}
