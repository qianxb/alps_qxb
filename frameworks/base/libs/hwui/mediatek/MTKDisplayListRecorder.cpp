...
NODE_CREATE11(RejectOp, char* name, void* displayList, float alpha, bool shouldClip, bool outlineEmpty,
    ...
    if (args->isQuick) {
        ...
        NODE_LOG("Rejected display list (%p, %s), displaylist %p, alpha %f, shouldClip %d, outlineEmpty %d, scaleX %f, scaleY %f",
            id, args->name, args->displayList, args->alpha, args->shouldClip, args->outlineEmpty, args->scaleX, args->scaleY);
    }
}
...
void DisplayListRecorder::recordImpl(MTK_BASE_PARAMS, const VectorDrawableOp& op) {
    ...
    ResolvedRenderState resolved(allocator(), *state.writableSnapshot(), op, false);
...
NODE_CREATE5(BitmapMeshOp, const SkBitmap* bitmap, int meshWidth, int meshHeight,
    Rect* localBounds, Rect* resolved) {
...
NODE_CREATE4(BitmapRectOp, const SkBitmap* bitmap, Rect *src, Rect* localBounds, Rect* resolved) {
...
NODE_CREATE2(ColorOp, int color, SkXfermode::Mode  mode) {
...
NODE_CREATE5(RectOp, Rect* localBounds, int style, int isAntiAlias, int xfermode, int color) {
...
void DisplayListRecorder::recordImpl(MTK_BASE_PARAMS, const RectOp& op) {
    ...
    args->style = op.paint->getStyle();
    args->isAntiAlias = op.paint->isAntiAlias();
    args->xfermode = PaintUtils::getXfermodeDirect(op.paint);
    args->color = op.paint->getColor();
}
...
void DisplayListRecorder::recordImpl(MTK_BASE_PARAMS, const TextureLayerOp& op) {
    ...
    args->width = op.unmappedBounds.getWidth();
    args->height = op.unmappedBounds.getHeight();
    args->textureWidth = op.layer->getWidth();
    args->textureHeight = op.layer->getHeight();
    args->alpha = op.layer->getAlpha();
    args->isIdentity = op.layer->getTransform().isIdentity();
    args->isBlend = op.layer->isBlend();
    args->isTextureLayer = true;
}

NODE_CREATE4(BitmapOp, const SkBitmap* bitmap, bool entry, Rect* localBounds, Rect* resolved) {
...
NODE_CREATE3(PatchOp, const SkBitmap* bitmap, bool entry, Rect* localBounds) {
...
void DisplayListRecorder::recordImpl(MTK_BASE_PARAMS, const MTKClipRegionOp& op) {
    SETUP_NODE(ClipRegionOp);
    int count = state.save(SaveFlags::MatrixClip);
    state.concatMatrix(op.localMatrix);
    state.clipRegion(op.region, op.op);
    args->bounds = COPY(SkIRect, op.region->getBounds());
    args->clipRect = COPY(Rect, state.currentSnapshot()->getClipArea().getClipRect());
    args->clipRegion = regionToChar(state.currentSnapshot()->getClipArea().getClipRegion());
    state.restoreToCount(count);
}
...
bool CanvasStateBuilding::clipRect(float left, float top, float right, float bottom, SkRegion::Op op) {
...
bool CanvasStateRecording::clipRect(float left, float top, float right, float bottom, SkRegion::Op op) {
...
bool CanvasStateRecording::clipPath(const SkPath* path, SkRegion::Op op) {
...
bool CanvasStateRecording::clipRegion(const SkRegion* region, SkRegion::Op op) {
    ADD_RETURN(mCanvasState.clipRegion(region, op),
        alloc().create_trivial<MTKClipRegionOp>(
            *mCanvasState.currentSnapshot()->transform,
            region, op));
}
