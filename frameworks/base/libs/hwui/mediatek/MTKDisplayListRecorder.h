...
struct MTKClipRectOp : RecordedOp {
        MTKClipRectOp(const Matrix4& localMatrix, float left, float top, float right, float bottom, SkRegion::Op op)
            : RecordedOp(RecordedOpId::MTKClipRectOp, Rect(), localMatrix, nullptr, nullptr)
            , area(left, top, right, bottom)
            , op(op) {}
    const Rect area;
    const SkRegion::Op op;
};

struct MTKClipPathOp : RecordedOp {
        MTKClipPathOp(const Matrix4& localMatrix, const SkPath* path, SkRegion::Op op)
...
struct MTKClipRegionOp : RecordedOp {
        MTKClipRegionOp(const Matrix4& localMatrix, const SkRegion* region, SkRegion::Op op)
            : RecordedOp(RecordedOpId::MTKClipRegionOp, Rect(), localMatrix, nullptr, nullptr)
            , region(region)
            , op(op) {}
...
//    void addReject(CanvasState& state, RenderNode* rendernode, bool isQuick);
    ...
    void addClipRect(MTK_BASE_PARAMS, float left, float top, float right, float bottom, SkRegion::Op op);
...
public:
    ...
    void setProjectionPathMask(LinearAllocator& allocator, const SkPath* path) {
        mCanvasState.setProjectionPathMask(allocator, path);
    }
...
public:
    ...
    void setDirtyClip(bool opaque) { mCanvasState.setDirtyClip(opaque); }
